...
def train1(model, dataset, loss_fn, optimizer):
    # 定义 forward 函数
...
def train2(model2, dataset, loss_fn, optimizer2):
    # 定义 forward 函数
    def forward_fn(data, label):
        # 将数据载入模型
        logits = model2(data)
...
def tt2(model2, dataset, loss_fn):
    num_batches = dataset.get_dataset_size()
...
for t in range(EPOCH):
    print(f"Epoch {t+1}\n-------------------------------")
    train1(model, train_dataset, loss_fn, optimizer)      # 训练模型
    tt1(model, test_dataset, loss_fn)                   # 测试模型
print("finished!")

print(f"===== Training model without dropout ======")
for i in range(EPOCH):
    print(f"Epoch {i+1}\n-------------------------------")
    train2(model2, train_dataset, loss_fn, optimizer2)      # 训练模型
    tt2(model2, test_dataset, loss_fn)                   # 测试模型
print("finished!")
