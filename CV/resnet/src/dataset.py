...
def create_dataset1(dataset_path, do_train, repeat_num=1, batch_size=32, target="Ascend"):
    ...
    if device_num == 1:
        ds = de.Cifar10Dataset(dataset_path, num_parallel_workers=1, shuffle=True)
    else:
        ds = de.Cifar10Dataset(dataset_path, num_parallel_workers=1, shuffle=True,
...
def create_dataset2(dataset_path, do_train, repeat_num=1, batch_size=32, target="Ascend"):
    ...
    if target == "Ascend":
        ...
        # init()
        #print('get_rank(): ', get_rank(), ' get_group_size(): ', get_group_size())
        rank_id = 0     # get_rank()
        device_num = 1  # get_group_size()


    if device_num == 1:
        ds = de.ImageFolderDataset(dataset_path, num_parallel_workers=2, shuffle=True)
    else:
        ds = de.ImageFolderDataset(dataset_path, num_parallel_workers=2, shuffle=True,
                                   num_shards=device_num, shard_id=rank_id)
    ...
    ds = ds.map(operations=trans, input_columns="image", num_parallel_workers=2)
    ds = ds.map(operations=type_cast_op, input_columns="label", num_parallel_workers=2)
