...
from EvalCallBack import EvalCallBack, apply_eval
from resnetv2 import resnet18, resnet50
import mindspore.dataset as ds
import mindspore.nn as nn
from mindspore import Tensor, context, load_checkpoint, load_param_into_net, Model, set_seed, TimeMonitor
...
def get_dataset(cfg, sample_num=None):
    ...
    读取并处理数据
    '''
    de_dataset = create_dataset(cfg, sample_num=sample_num)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='MindSpore ResNet Example')
    ...
    network = resnet18(cfg.num_class)

    x = Tensor(np.ones([1, 3, 224, 224]), dtype=mstype.float32)
    print('net(images).shape: ', network(x).shape, x.dtype)

    data = d_test.create_dict_iterator().__next__()
    images = data['image']
    print(images.shape)
    print(images.dtype)
    print('*'*10)
    ...
    #ckpoint_cb = ModelCheckpoint(prefix="checkpoint_resnet_flower",
    #                             directory=args.ckpt_path, config=config_ck)
    #model.train(cfg.epoch_size, de_train, callbacks=[ckpoint_cb, ckpoint_cb, LossMonitor()],
    ...
    # ds_test = create_dataset(os.path.join(args.data_path, "test"), cfg.batch_size, 1)
