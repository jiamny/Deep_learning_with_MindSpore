...
class DecisionTree_CART:
    ...
    def fit(self, X, y):
        ...
        """
        self.n_classes_ = len(ms.ops.unique(y))  # classes are assumed to go from 0 to n-1
    ...
    def _gini(self, y):
        ...
        """
        m = y.shape[0]
        return 1.0 - sum((int(ms.ops.equal(y, c).sum().item()) // m) ** 2 for c in range(self.n_classes_))
    ...
    def _best_split(self, X, y):
        ...
        num_parent = [int(ms.ops.equal(y, c).sum().item()) for c in range(self.n_classes_)]
    ...
    def _grow_tree(self, X, y, depth=0):
        ...
        # largest population.
        num_samples_per_class = ms.Tensor([int(ms.ops.equal(y, i).sum().item()) for i in range(self.n_classes_)])
        predicted_class = ms.ops.argmax(num_samples_per_class)
...
if __name__ == "__main__":
    ...
    * Fit and predict with DecisionTree Object.
    ...
    x_test = ms.Tensor(x_test)
    y_test = ms.Tensor(y_test)

    classifier = DecisionTree_CART(max_depth=5)
    classifier.fit(x_train, y_train)
    y_predict = classifier.predict(x_test)
    y_predict = ms.Tensor([int(i.item()) for i in y_predict], dtype=mstype.int64)
    print(f'Accuracy: {accuracy_score(y_test.asnumpy(), y_predict.asnumpy())}')
